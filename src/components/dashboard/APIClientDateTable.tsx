...
function generateCellData(apiCalls: number, seed: number, clientSeed: number) {
  const rng = ((seed * 31 + clientSeed * 17) % 97) / 97;
  ...
  const prevRng = ((seed * 23 + clientSeed * 13 + 7) % 89) / 89;
...
interface APIClientDateTableProps {
  apis: APIData[];
}

/**
 * Inverted breakdown: rows are clients consuming a single API.
 */
export function APIClientDateTable({ apis }: APIClientDateTableProps) {
  ...
  const sortedClientRows = [...apis].sort((a, b) => b.currentCalls - a.currentCalls);

  return (
    <div className="mt-6">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-sm font-semibold text-foreground">Date-wise Client Breakdown</h4>
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-[10px] min-w-[700px]">
          <thead className="bg-muted/50 sticky top-0">
            <tr>
              <th className="px-3 py-2 text-left font-semibold text-muted-foreground border-r border-border min-w-[120px]">Client</th>
              {columns.map(col => (
          ...
          <tbody className="divide-y divide-border">
            {sortedClientRows.map(api => {
              const clientSeed = api.client.charCodeAt(0) + api.client.charCodeAt(api.client.length - 1);
              return (
                <tr key={api.id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-3 py-2 font-medium text-foreground border-r border-border truncate max-w-[140px]" title={api.client}>
                    {api.client}
                  </td>
                  {columns.map(col => {
                    const cell = generateCellData(api.currentCalls, col.seed, clientSeed);
